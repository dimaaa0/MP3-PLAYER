...
export default function MiniPlayer() {
  ...
  useEffect(() => {
    if (track && favorites.length > 0) {
      const isFav = favorites.some(
        (fav: any) => fav.name === track.name && fav.artist === track.artist,
      );
  ...
  const handleAddFavorite = async (
    ...
    const res = await fetch("/api/favorites", {
      ...
      body: JSON.stringify({ name: title, artist, imageUrl, duration }),
    });
  ...
  if (!track) {
    return (
      <div className="h-screen w-screen bg-[#121212] flex items-center justify-center text-gray-500 p-6 text-center">
        <p className="animate-pulse text-xs uppercase tracking-widest">
    ...
    <div className="h-screen w-screen bg-[#121212] text-white flex flex-col p-8 overflow-hidden relative select-none">
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="mt-auto flex w-full justify-center items-center pb-2">
          ...
          >
            <Star
              className={`w-7 h-7 transition-all ${isFavorited ? "text-yellow-400 fill-yellow-400 scale-110" : ""}`}
            />
          </button>
        </div>
          ...
          <>
            <div className="w-full h-1 bg-white/40 rounded-full mt-4">
              <div
                ...
                style={{
                  maxWidth: "100%",
                  width: `${(track.goingTime / formatGoingTime(track.duration)) * 100}%`,
                }}
              >
