...
const MusicList = ({ music, inputValue, recentCategory }: MusicListProps) => {
    ...
    const handlePopOut = () => {
        const width = 400;
        const height = 500; // Оставляем запас под системную рамку
        ...
        const playerWindow = window.open('/pages/mini-player', 'MusicStreamPlayer', features);
        ...
        if (currentTrack) {
            channel.postMessage({
                ...
                track: {
                    name: currentTrack.name,
                    artist: currentTrack.artist,
                    imageUrl: currentTrack.imageUrl, // убедитесь, что в currentTrack есть это поле
                    isLoadingVideo: isLoadingVideo
                }
            });
        }
